...
const CollectorAssignmentsPage = () => {
      ...
      case 'Assigned':
        return (
          <button
            ...
            className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-slate-900 bg-teal-600 hover:bg-teal-700 shadow-sm flex items-center justify-center space-x-1.5 transition-all"
          >
      ...
      case 'Accepted':
        return (
          <button
            ...
            className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-slate-900 bg-sky-600 hover:bg-sky-500 shadow-glow-blue flex items-center justify-center space-x-1.5 transition-all"
          >
      ...
      case 'Collecting':
        return (
          <button
            ...
            className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-slate-900 bg-indigo-600 hover:bg-indigo-500 flex items-center justify-center space-x-1.5 transition-all"
          >
      ...
      case 'Collected':
        return (
          <button
            ...
            className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-slate-900 bg-emerald-600 hover:bg-emerald-500 flex items-center justify-center space-x-1.5 transition-all"
          >
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black tracking-tight text-slate-900">Active Biohazard Pickups</h1>
          ...
          {requests.map((req) => {
            ...
            return (
              <GlassCard key={req._id} className="p-6">
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                  <div>
                    <div className="flex items-center space-x-2.5 mb-2">
                      <span className="font-mono text-sm font-bold text-teal-600 bg-teal-500/10 px-2.5 py-1 rounded border border-teal-500/20">
                        {req.requestId}
                      </span>
                      <PriorityBadge priority={req.priority} />
                      <StatusBadge status={req.status} />
                    </div>
                    <h3 className="text-lg font-bold text-slate-900">{req.hospital?.name}</h3>
                  ...
                  <div className="text-right">
                    <span className="text-xs text-slate-500">Total Biohazard Payload</span>
                ...
                <div className="p-4 rounded-xl bg-white/60 border border-slate-200/50 mb-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
                    {req.wasteRecords?.map((w) => (
                      <div
                        key={w._id}
                        className="p-2.5 rounded-lg bg-slate-50/60 border border-slate-200/40 text-xs flex flex-col justify-between"
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-mono font-bold text-teal-700">{w.wasteId}</span>
                          <span className="font-bold text-slate-900">{w.quantity} {w.unit}</span>
                        </div>
                        <div className="text-[11px] text-slate-500">
                          {w.category?.name} • <span className="text-slate-600">{w.department}</span>
                        </div>
                        {w.bin && (
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-800">
                  <div className="text-xs text-slate-500">
                    Requested by: <strong className="text-slate-700">{req.requestedBy?.name}</strong> on{' '}
                  ...
                  <div className="w-full sm:w-auto flex items-center space-x-3">
                    <Link
                      ...
                      className="px-4 py-2 rounded-xl text-xs font-semibold text-slate-600 hover:text-slate-900 bg-slate-50 hover:bg-slate-100 border border-slate-200 transition-colors"
                    >
                      Audit Trail
                    </Link>
      ...
      >
        <form onSubmit={handleConfirmStatusChange} className="space-y-4">
          <p className="text-xs text-slate-600 leading-relaxed">
            Updating status of <strong className="text-teal-600 font-mono">{actionModal.request?.requestId}</strong> to{' '}
            <strong className="text-slate-900 uppercase font-bold">{actionModal.targetStatus}</strong>.
